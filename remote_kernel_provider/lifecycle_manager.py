...
class BaseKernelLifecycleManagerABC(with_metaclass(abc.ABCMeta, object)):
    ...
    def _enforce_limits(self, **kwargs):
        ...
        # TODO - this will need to move to MappingKernelManager subclass and remain in EG...
        ...
        # max_kernels_per_user = self.kernel_manager.app_config.get('max_kernels_per_user', -1)
...
class RemoteKernelLifecycleManager(with_metaclass(abc.ABCMeta, BaseKernelLifecycleManagerABC)):
    ...
    def __init__(self, kernel_manager, lifecycle_config):
        ...
        self.tunneled_connection_info = None    # Contains the destination connection info when tunneling in use
        ...
        if self.local_proc:
            ...
            if poll_result and poll_result > 0:
                ...
                error_message = "Error occurred during launch of KernelID: {}.  " \
                                "Check server log for more information.".format(self.kernel_id)
                self.local_proc = None
    ...
    def _setup_connection_info(self, connect_info):
        ...
        if tunneling_enabled is True:
            ...
            self.tunneled_connection_info = dict(connect_info)
            ...
            if 'comm_port' in connect_info:
                ...
                self.log.debug("Established server communication to: {}:{} for KernelID '{}' via tunneled port "
            ...
            if 'comm_port' in connect_info:
                ...
                self.log.debug("Established server communication to: {}:{} for KernelID '{}'".
        ...
        if 'comm_port' not in connect_info:
            self.log.debug("Server communication port has NOT been established for KernelID '{}' (optional).".
                           format(self.kernel_id))
    ...
    def _update_connection(self, connect_info):
        ...
        if connect_info:
            ...
            self.connection_info = connect_info
